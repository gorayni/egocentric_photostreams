...
def write_results(results, csv_filepath):
    with open(csv_filepath, 'w') as csv_file:
        for img_path, label, predictions in results:
            predictions_csv = ','.join(['%.10f' % p for p in predictions[0]])
            csv_file.write("{},{},{}".format(img_path, label, predictions_csv))
            csv_file.write('\n')
...
if __name__ == '__main__':
    ...
    results_dir = os.path.realpath(os.path.join(args.results_dir, args.network))
    if args.network == 'vgg-16':
        model = edict({'best_weights': weights_dir + '/weights.vgg-16.phase_2.fold_{}.best.' + backend + '.hdf5',
                       'name': 'vgg-16',
                       'img_width': 224,
                       'img_height': 224,
        ...
        model = edict({'best_weights': weights_dir + '/weights.resNet50.phase_2.fold_{}.best.' + backend + '.hdf5',
             'name': 'resNet50',
             'img_width': 224,
             'img_height': 224,
        ...
        model = edict({'best_weights': weights_dir + '/weights.inceptionV3.phase_2.fold_{}.best.' + backend + '.hdf5',
             'name': 'inceptionV3',
             'img_width': 299,
             'img_height': 299,
